...
Now we test $\phi_{SM}$ through the algorithm \cite{algo} with the function $BouclePerdante$ that keeps in memory, all the possibility explored in order to help the interpolant.\\
...
For $nb_{robot} = 4$ and $size_{ring} = 2$ we should find Unsat, if we do not, it's because the algorithm calls the interpolant in the first place and so there are simplification and one $AsyncPost$ \cite{algo} is done wrong.
