...
while cpt < 10:
        ...
        while continuer:
                ...
                tmpAndInterpolant.append(AsyncPost(taille_anneau, nb_robots, p, s, t, pk[0], sk[0], tk[0], phi))

                if k > 1:
                        for i in range(1, k):
                                print("Pass before %s" % i)
                                tmpAndContext.append(AsyncPost(taille_anneau, nb_robots, pk[i-1], sk[i-1], tk[i-1], pk[i], sk[i], tk[i], phi))
                                print("Pass after %s" % i)

                while (not satisfiable) or (taille_boucle < taille_boucle_max):
                        tmpAndContextBis = []
                        taille_boucle = taille_boucle + 1
                        tmpAndContextBis.append(BouclePerdante(taille_anneau, pk[-1], sk[-1], tk[-1], taille_boucle, phi))
